...
const WhatsAppButton = ({ size = "md" }: { size?: "sm" | "md" | "lg" }) => {
  const sizeClasses = {
  ...
  return (
    <motion.button 
      className={`${sizeClasses[size]} whatsapp-bg rounded-full flex items-center justify-center shadow-lg transition-all duration-300 z-10 focus:outline-none focus:ring-4 whatsapp-ring`}
      ...
      >
        ...
        <div className="flex items-center gap-2 sm:gap-2.5 mb-2 sm:mb-2.5 min-w-0">
          ...
          <div className="flex-1 min-w-0 flex flex-col items-center justify-center large-card-header ward-bg rounded-md ward-box-padding">
            <p className="text-[7px] sm:text-[8px] text-white mb-zero font-semibold uppercase tracking-wide text-center leading-tight">{t.Assembly}</p>
    ...
    >
      ...
      <div className="flex items-center gap-2 sm:gap-2.5 mb-2 sm:mb-2.5 min-w-0">
        ...
        <div className="flex-1 min-w-0 flex flex-col items-center justify-center voter-card-header-height ward-bg rounded-md ">
          <p className="text-xs sm:text-sm md:text-base text-white mb-0.5 font-semibold uppercase tracking-wide text-center">{t.Assembly}</p>
          <p className="text-sm sm:text-xs md:text-sm font-bold text-white break-words text-cente">{person.wardNumber || "N/A"}</p>
        </div>
        {/* EPIC Number */}
        <div className="flex-1 min-w-0 flex flex-col items-center justify-center voter-card-header-height bg-gradient-to-br from-peach-100 to-peach-200 rounded-md ">
          <p className="text-xs sm:text-sm md:text-base text-black mb-0.5 font-semibold uppercase tracking-wide text-center">{t.epicNo}</p>
          <p className="text-sm sm:text-xs md:text-sm font-bold text-black break-words text-cente">{person.epicNo || "N/A"}</p>
        </div>
      </div>
      ...
      <div className="space-y-1.5 sm:space-y-2 md:space-y-2.5 mb-2 sm:mb-2.5 md:mb-3">
        ...
        <div className="bg-slate-50 rounded-md p-2 sm:p-2.5 space-y-1.5 sm:space-y-2 border border-slate-100">
          <div className="grid grid-cols-2 gap-x-2 sm:gap-x-2.5 md:gap-x-3">
            {/* Column 1 */}
            <div className="min-w-0 space-y-1.5 sm:space-y-2">
              ...
              <div>
                <p className="text-[8px] sm:text-[9px] text-slate-600 mb-0.5 font-semibold uppercase tracking-wide">{t.ward}</p>
                <p className="text-[9px] sm:text-[10px] md:text-xs font-bold text-slate-900 break-words leading-snug">{person.ward || "N/A"}</p>
              </div>
              <div>

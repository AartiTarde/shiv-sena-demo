...
export default function DeletedSearchPage() {
  ...
  const totalPages = Math.ceil(deletedResults.length / ITEMS_PER_PAGE);
  const startIndex = (deletedCurrentPage - 1) * ITEMS_PER_PAGE;
  ...
  return (
    <div className="bg-peach-50 min-h-full w-full overflow-x-hidden overflow-y-auto">
      ...
      >
        ...
        >
          <h1 className="text-lg sm:text-xl md:text-2xl font-bold text-slate-900">Deleted After VS</h1>
          <button
            onClick={() => {
          ...
          <>
            <div className="mb-2 sm:mb-3 md:mb-4">
              <p className="text-xs sm:text-sm md:text-base text-slate-600">
                Showing <span className="font-bold text-slate-900">{deletedResults.length}</span> result{deletedResults.length !== 1 ? "s" : ""}
              </p>
            </div>
